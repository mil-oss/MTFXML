...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:inf="urn:mtf:mil:6040b:appinfo" exclude-result-prefixes="xs" version="2.0">
    <xsl:output method="xml" indent="yes"/>

    <xsl:variable name="dirpath" select="'../../XSD/NIEM_MTF/'"/>
    <xsl:variable name="ALLMTF" select="document(concat($dirpath, 'NIEM_MTF.xsd'))/xs:schema"/>
    ...
    <xsl:variable name="msgnodes">
        <xsl:for-each select="$ALLMTF/*:element[xs:annotation/xs:appinfo/*:Msg/@mtfid]">
            
        <xsl:for-each select="$ALLMTF//*[@ref | @base | @type]">
            ...
            <xsl:for-each select="$ref-xsd-template/*">
                <xsl:copy>
                    ...
                    <xs:annotation>
                        ...
                        <xs:appinfo>
                            <inf:Msg mtfname="{$message/xs:annotation/xs:appinfo/*:Msg/@mtfname}" mtfid="{$msgid}"/>
                        </xs:appinfo>
                    </xs:annotation>
