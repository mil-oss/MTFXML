...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" exclude-result-prefixes="xs" version="2.0">
    ...
    <xsl:template name="extractallsubsetxsd">
        ...
        <xsl:result-document href="{concat($listOutDir,'/msgmap.xml')}">
            <Messages>
    ...
    <xsl:template match="*" mode="ssidentity">
        <xsl:copy copy-namespaces="no">
            <xsl:apply-templates select="@*" mode="ssidentity"/>
            <xsl:apply-templates select="text()" mode="ssidentity"/>
            <xsl:apply-templates select="*" mode="ssidentity"/>
        </xsl:copy>
    </xsl:template>
    ...
    <xsl:template match="text()" mode="ssidentity">
