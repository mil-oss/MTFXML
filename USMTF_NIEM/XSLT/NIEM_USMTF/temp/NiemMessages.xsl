...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xsd="http://www.w3.org/2001/XMLSchema" xmlns:term="http://release.niem.gov/niem/localTerminology/3.0/"
    ...
    <!--Outputs-->
    <xsl:variable name="messagemapsoutput" select="concat($srcdir, 'NIEM_MTF_Messagemaps.xml')"/>
    ...
    <xsl:template match="xsd:element">
        ...
        <xsl:variable name="niemelementnamevar">
            <xsl:choose>
                ...
                <xsl:when test="$niemnamevar = 'ContactSummary'">
                    <xsl:text>ContactSummarySet</xsl:text>
                </xsl:when>
                <xsl:when test="$niemnamevar = 'TargetIdentification'">
                    <xsl:text>TargetIdentificationSet</xsl:text>
                </xsl:when>
                <xsl:when test="$niemnamevar = 'Association'">
    ...
    <xsl:variable name="messagelements">
        <xsl:for-each select="$messagemaps//Sequence/Element">
            ...
            <xsl:choose>
                <xsl:when test="$niem_segments_map//*[@niemelementname = $n]"/>
