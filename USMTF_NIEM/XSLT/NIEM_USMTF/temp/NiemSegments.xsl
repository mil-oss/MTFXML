...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xsd="http://www.w3.org/2001/XMLSchema" xmlns:term="http://release.niem.gov/niem/localTerminology/3.0/"
    ...
    <xsl:variable name="srcdir" select="'../../XSD/NIEM_MTF_1_NS/'"/>
    <xsl:variable name="niem_fields_map" select="document(concat($srcdir, 'NIEM_MTF_Fieldmaps.xml'))/*"/>
    <xsl:variable name="niem_composites_map" select="document(concat($srcdir, 'NIEM_MTF_Compositemaps.xml'))/*"/>
    <xsl:variable name="niem_sets_map" select="document(concat($srcdir, 'NIEM_MTF_Setmaps.xml'))/*"/>
    <xsl:variable name="niem_segments_map" select="document(concat($srcdir, 'NIEM_MTF_Segmentmaps.xml'))/*"/>
    ...
    <!--Outputs-->
    <xsl:variable name="segmentmapsoutput" select="concat($srcdir, 'NIEM_MTF_Segmentmaps.xml')"/>
    ...
    <xsl:template match="xsd:element" mode="segmentglobal">
        ...
        <Segment mtfname="{@name}" messagename="{$messagename}" niemcomplextype="{$niemcomplextype}" niemelementname="{$niemelementnamevar}" niemelementdoc="{$niemelementdocvar}" mtfdoc="{$mtfdoc}"
