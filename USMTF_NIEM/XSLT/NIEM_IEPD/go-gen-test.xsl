...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:exsl="http://exslt.org/common" xmlns:xs="http://www.w3.org/2001/XMLSchema" exclude-result-prefixes="xs" version="1.0">
    ...
    <xsl:template name="maketests">
        ...
        <xsl:apply-templates select="//xs:schema/xs:complexType[@name = $roottype]/*" mode="maketest">
            <xsl:with-param name="pckgname" select="$pckgname"/>
            <xsl:with-param name="testdata" select="document($testpath)"/>
        </xsl:apply-templates>
    ...
    <xsl:template name="teststart">
        ...
        <xsl:value-of select="concat($in, 'xsdprov.WgetFile(testinstances[', $qt, 'test_instance.xml', $qt, ']', $cm, $qt, $testurl, $qt, ')', $cr)"/>
    ...
    <xsl:template match="xs:element[@ref]" mode="maketest">
        <xsl:param name="pckgname"/>
        <xsl:param name="testdata"/>
        <xsl:param name="dotpath"/>
        ...
        <xsl:if test="exsl:node-set($testdata)//*[name() = $r]">
            ...
            <xsl:choose>
                <xsl:when test="not(//xs:schema/xs:complexType[@name = $ty]//xs:element[@ref]) and not(//xs:schema/xs:complexType[@name = $b]//xs:element[@ref])">
                    ...
                    <xsl:variable name="testval">
                        <xsl:value-of select="normalize-space(exsl:node-set($testdata)//*[name() = $root][1]//*[name() = $parent][1]//*[name() = $r][1])"/>
                    </xsl:variable>
                    <xsl:if test="string-length($testval) &gt; 0">
                        <xsl:value-of select="concat($in, $in, $in, 'Expect(', $pckgname, $dotpath, '.', $r, $ary1, '.Value', ').To(Equal(', $qt, $testval, $qt, '))', $cr)"/>
                    </xsl:if>
                    <xsl:apply-templates select="//xs:schema/xs:complexType[@name = $ty]/*" mode="maketest">
                        <xsl:with-param name="pckgname" select="$pckgname"/>
                        <xsl:with-param name="testdata" select="$testdata"/>
                ...
                <xsl:otherwise>
                    ...
                    <xsl:choose>
                        <xsl:when test="//xs:schema/xs:complexType[@name = $parentType]//xs:element[@ref]">
                            <xsl:apply-templates select="//xs:schema/xs:complexType[@name = $ty]/*" mode="maketest">
                                <xsl:with-param name="pckgname" select="$pckgname"/>
                                <xsl:with-param name="testdata" select="$testdata"/>
                                <xsl:with-param name="dotpath" select="concat($dotpath, '.', $r)"/>
                        ...
                        <xsl:otherwise>
                            <xsl:value-of select="concat('g.It(', $qt, 'Must Have ', $node/@name, $qt, $cm, 'func() {', $cr)"/>
                            <xsl:apply-templates select="//xs:schema/xs:complexType[@name = $ty]/*" mode="maketest">
                                <xsl:with-param name="pckgname" select="$pckgname"/>
                                <xsl:with-param name="testdata" select="$testdata"/>
                                <xsl:with-param name="dotpath" select="concat($dotpath, '.', $r)"/>
    ...
    <xsl:template match="xs:attribute[@ref]" mode="maketest">
        ...
        <xsl:if test="exsl:node-set($testdata)//@*[name() = $r]">
            ...
            <xsl:variable name="testval">
                <xsl:choose>
                    <xsl:when test="string-length($parent) = 0">
                        <xsl:value-of select="normalize-space(exsl:node-set($testdata)/*/@*[name() = $r])"/>
                    </xsl:when>
                    <xsl:when test="string-length($root) &gt; 0">
                        <xsl:value-of select="normalize-space(exsl:node-set($testdata)//*[name() = $root][1]//*[name() = $parent][1]//@*[name() = $r][1])"/>
                    </xsl:when>
                    <xsl:otherwise>
                        <xsl:value-of select="normalize-space(exsl:node-set($testdata)//*[name() = $parent][1]//@*[name() = $r][1])"/>
                    </xsl:otherwise>
                </xsl:choose>
            </xsl:variable>
            <xsl:variable name="parentType" select="//xs:schema/xs:element[@name = $parent]/@type"/>
            <xsl:choose>
                <xsl:when test="string-length($parent) = 0">
                    <xsl:value-of select="concat('g.It(', $qt, 'Must Have ', $node/@name, $qt, $cm, 'func() {', $cr)"/>
                    <xsl:value-of select="concat($in, $in, $in, 'Expect(', $pckgname, $dotpath, '.', $rn, '.Value', ').To(Equal(', $qt, $testval, $qt, '))', $cr)"/>
                    <xsl:value-of select="concat($in, $in, '})', $cr)"/>
                </xsl:when>
                <xsl:otherwise>
                    <xsl:value-of select="concat($in, $in, $in, 'Expect(', $pckgname, $dotpath, '.', $rn, '.Value', ').To(Equal(', $qt, $testval, $qt, '))', $cr)"/>
    ...
    <xsl:template match="xs:attributeGroup[@ref]" mode="maketest">
        ...
        <xsl:apply-templates select="//xs:schema/xs:attributeGroup[@name = $an]/xs:attribute" mode="maketest">
            <xsl:with-param name="pckgname" select="$pckgname"/>
            <xsl:with-param name="testdata" select="$testdata"/>
            <xsl:with-param name="dotpath" select="$dotpath"/>
        </xsl:apply-templates>
