...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" exclude-result-prefixes="xs" version="2.0">
    ...
    <xsl:variable name="schemaMaps">
        <xsl:copy-of select="document(concat($basepath, 'Maps/NIEM_MTF_Fieldmaps.xml'))/Fields"/>
        <xsl:copy-of select="document(concat($basepath, 'Maps/NIEM_MTF_Compositemaps.xml'))/Composites"/>
        <xsl:copy-of select="document(concat($basepath, 'Maps/NIEM_MTF_Sgmntmaps.xml'))/Segments"/>
        <xsl:copy-of select="document(concat($basepath, 'Maps/NIEM_MTF_Setmaps.xml'))/Sets"/>
        <xsl:copy-of select="document(concat($basepath, 'Maps/NIEM_MTF_Msgsmaps.xml'))/Messages"/>
    </xsl:variable>
    ...
    <xsl:template name="makeJson">
        ...
        <xsl:for-each select="$schemaMap/*">
            ...
            <xsl:if test="count(preceding-sibling::*[@niemelementname = $jname]) = 0">
