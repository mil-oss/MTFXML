 ...
 */
-->
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:inf="urn:mtf:mil:6040b:appinfo" xmlns:exsl="http://exslt.org/common"
   ...
   -->
    ...
    <xsl:template match="/">
        <xsl:call-template name="xsdjsonmain"/>
    </xsl:template>

    <xsl:template name="xsdjsonmain">
    ...
    <xsl:template match="xs:appinfo" mode="tojson">
        <xsl:if test="*">
            <xsl:variable name="inc">
                <xsl:if test="preceding-sibling::xs:appinfo">
                    <xsl:text>2</xsl:text>
                </xsl:if>
            </xsl:variable>
            <xsl:value-of select="concat($cm, $q, 'appinfo',$inc, $q, $c, $lbr, $lb)"/>
            ...
            <xsl:for-each select="*">
                ...
                <xsl:value-of select="concat($q, name(), $incr, $q, $c, $lb)"/>
                ...
                <xsl:value-of select="$rb"/>
                <xsl:value-of select="$rb"/>
                <xsl:if test="following-sibling::inf:*">
                    <xsl:value-of select="$cm"/>
                    <xsl:value-of select="$lb"/>
                </xsl:if>
            </xsl:for-each>
