...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" exclude-result-prefixes="xs" version="2.0">
    ...
    <xsl:variable name="refsrc" select="'../../XSD/NIEM_MTF/NIEM_MTF.xsd'"/>
    ...
    <xsl:template match="xs:simpleType">
        ...
        <xsl:if test="$maxlen">
            <Test valid="true" value="{upper-case(substring($longtext,0,ceiling($maxlen div 2)))}"/>
        ...
        <xsl:if test="$minlen">
            <Test valid="true" value="{upper-case(substring($longtext,0,$minlen))}"/>
            <Test valid="false" value="{substring($longtext,0,$minlen -1)}"/>
        </xsl:if>
        <xsl:if test="$mtypes/*[@minlen]">
            <Test valid="true" value="{upper-case(substring($longtext,0,$mtypes/*[@minlen]))}"/>
