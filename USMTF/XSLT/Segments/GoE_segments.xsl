...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
                    ...
                    <xsd:extension base="set:GeneralTextType">
                        <xsd:sequence>
                            <xsd:element name="GentextTextIndicator"
                                type="field:AlphaNumericBlankTextSimpleType" minOccurs="1"
                                fixed="{replace($TextInd,$apos,'')}"/>
                            <xsd:element ref="field:FreeText" minOccurs="1"/>
                        </xsd:sequence>
                    </xsd:extension>
                </xsd:complexContent>
            </xsd:complexType>
    ...
    <xsl:template match="/">
        <xsl:result-document href="../../XSD/GoE_Schema/GoE_segments.xsd">
            <xsd:schema xmlns="urn:mtf:mil:6040b:segments"
                ...
                <!--<xsl:copy-of select="$deconflict_complex_types"/>-->
    ...
    <xsl:template match="xsd:element[contains(@name, '_')][not(starts-with(@name, 'GeneralText_'))]">
        <xsl:variable name="match_new_name">
            <xsl:call-template name="matchChange">
                <xsl:with-param name="matchType">
                    <xsl:choose>
                        <xsl:when test="starts-with(@base, 's:')">
        ...
        <xsl:choose>
            <xsl:when test="string-length($match_new_name) > 0">
                <xsd:element ref="{concat('set:',$match_new_name)}">
                    <xsl:copy-of select="xsd:annotation"/>
                </xsd:element>
            </xsl:when>
            <xsl:otherwise>
    ...
    <xsl:template match="xsd:extension[@base][not(@base = 's:GeneralTextType')]">
        ...
        <xsl:copy copy-namespaces="no">
            ...
            <xsl:attribute name="base">
                <xsl:choose>
                    <xsl:when test="$sets/xsd:schema/xsd:complexType[@name = $match_new_name]">
                        <xsl:value-of select="concat('set:', $match_new_name)"/>
                    </xsl:when>
                    <xsl:when test="$sets/xsd:schema/xsd:simpleType[@name = $match_new_name]">
                        <xsl:value-of select="concat('set:', $match_new_name)"/>
                    </xsl:when>
                    <xsl:otherwise>
                        <xsl:value-of select="$match_new_name"/>
