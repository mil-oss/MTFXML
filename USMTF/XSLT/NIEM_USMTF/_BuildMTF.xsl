...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" exclude-result-prefixes="xs" xmlns:ism="urn:us:gov:ic:ism" xmlns:sch="http://purl.oclc.org/dsdl/schematron" xmlns:inf="urn:mtf:mil:6040b:appinfo" version="2.0">
                    ...
                    <xsl:if test="count(preceding-sibling::*[@niemelementname = $n][@niemtype = $t]) = 0">
                        <xs:element name="{@niemelementname}">
                            ...
                            <xs:annotation>
                                ...
                                <xs:appinfo>
                                    <xsl:apply-templates select="info/*[1]" mode="identity"/>
                                </xs:appinfo>
                            </xs:annotation>
                        </xs:element>
    ...
    <xsl:variable name="setfields">
        <!--Add New Elements for Field Groups -->
        <xsl:for-each select="$niem_sets_map//Sequence[@name = 'GroupOfFields']">
            ...
            <xsl:variable name="settype">
                <xsl:value-of select="ancestor::Set/@niemtype"/>
            </xsl:variable>
            ...
            <xsl:variable name="fgname" select="@niemname"/>
            ...
            <xs:element name="{$fgname}" type="{concat($fgname,'Type')}" nillable="true">
                ...
                <xs:annotation>
                    ...
                    <xs:appinfo>
                        <inf:Composite name="{$fgname}" version="C.0.01.00" date="April 2020" setname="{$settype}"/>
                    </xs:appinfo>
                </xs:annotation>
            </xs:element>
    ...
    <xsl:variable name="setsxsd">
        <xsl:for-each select="$niem_sets_map/Set">
            ...
            <xsl:variable name="fgseq">
                <xsl:value-of select="@fieldgroupseq"/>
            </xsl:variable>
                                    ...
                                    <xsl:when test="@name = 'GroupOfFields'">
                                        <xs:element ref="{@niemname}">
                                            ...
                                            <xs:annotation>
                                                <xs:documentation>
                                                    <xsl:call-template name="breakIntoWords">
                                                        <xsl:with-param name="string" select="@niemname"/>
                                                    </xsl:call-template>
                                                </xs:documentation>
                                                <xs:appinfo>
                                                    <inf:Composite positionName="FIELD GROUP" fieldseq="{position()}" setname="{$settype}"/>
                                                </xs:appinfo>
                                            </xs:annotation>
                                        </xs:element>
                                    </xsl:when>
                                        ...
                                        <xsl:for-each select="*">
                                            <xsl:choose>
                                                <xsl:when test="name() = 'Element'">
                                                    ...
                                                    <xs:element ref="{@niemelementname}">
                                                        ...
                                                        <xs:annotation>
                                                            <xs:documentation>
                                                                <xsl:choose>
                                                                    <xsl:when test="@niemelementname = 'UnformattedFreeText'">
                                                ...
                                                <xsl:when test="name() = 'Sequence'">
                                                    <!--<xsl:if test="@name = 'GroupOfFields'">-->
                                                    <xs:element ref="{@niemname}">
                                                        <xsl:copy-of select="@minOccurs"/>
                                                        <xsl:copy-of select="@maxOccurs"/>
                                                        <xs:annotation>
        ...
        <xsl:for-each select="$niem_segments_map//Element">
            ...
            <xsl:choose>
                ...
                <xsl:otherwise>
                    ...
                    <xsl:variable name="setinfo" select="$niem_sets_map[@niemtype=$t]/info/inf:Set"/>
                    ...
                    <xs:element name="{@niemelementname}">
                        ...
                        <xs:annotation>
                            ...
                            <xsl:choose>
                                <xsl:when test="$setinfo">
                                    <xs:appinfo>
                                        <xsl:copy-of select="$setinfo"/>
                                    </xs:appinfo>
                                </xsl:when>
                        ...
                        <xsl:when test="@niemelementname">
                            <xs:element name="{@niemelementname}">
                                ...
                                <xs:annotation>
                                    ...
                                    <xs:appinfo>
                                        <xsl:choose>
                                            <xsl:when test="@niemelementname = 'RemarksSet'">
                                                <xsl:copy-of select="$all_set_elements_map//*[@niemelementname = 'RemarksSet']/info/inf:Set"/>
                                            </xsl:when>
                                            <xsl:otherwise>
                                                <xsl:apply-templates select="info/*[1]" mode="identity"/>
                                            </xsl:otherwise>
    ...
    <!--appinfo for global components should not include contextual information such as sequence etc.-->
    <xsl:template match="info/*" mode="globalinfo">
        ...
        <xsl:copy>
            ...
            <!--<xsl:copy-of select="@columnName" copy-namespaces="no"/>-->
            <xsl:copy-of select="@columnar" copy-namespaces="no"/>
            <!--<xsl:copy-of select="@fieldgroupseq" copy-namespaces="no"/>-->
            ...
            <xsl:copy-of select="@setname" copy-namespaces="no"/>
            <xsl:copy-of select="@segmentname" copy-namespaces="no"/>-->
            <xsl:apply-templates select="*" mode="copy"/>
        </xsl:copy>
    </xsl:template>
    <!--appinfo for referenced components must include contextual information, which is required to 
        parse MTF Slash delimited  Format using XSLT.-->
    <xsl:template match="info/*" mode="refinfo">
        ...
        <xsl:copy>
            ...
            <xsl:copy-of select="@date" copy-namespaces="no"/>
            <xsl:copy-of select="@remark" copy-namespaces="no"/>
            <xsl:copy-of select="@initialPosition" copy-namespaces="no"/>
            <!--<xsl:copy-of select="@segseq" copy-namespaces="no"/>-->
            <xsl:copy-of select="@setseq" copy-namespaces="no"/>
            <xsl:copy-of select="@fieldseq" copy-namespaces="no"/>
            <xsl:copy-of select="@setname" copy-namespaces="no"/>
