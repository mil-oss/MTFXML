...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:inf="urn:mtf:mil:6040b:appinfo" version="2.0">
   ...
   accordance with provisions of DOD Directive 5230.25.</xsl:text>
    ...
    <xsl:template match="*:schema/*:complexType" mode="setglobal">
        ...
        <xsl:variable name="annot">
            <xsl:choose>
                <xsl:when test="$mtfname = 'SetBaseType'">
                    <inf:Set setid="SETBASE" name="SET BASE" version="B.1.01.04"/>
                </xsl:when>
                <xsl:otherwise>
                    <xsl:apply-templates select="*:annotation"/>
                </xsl:otherwise>
        ...
        <xsl:variable name="niemtypedocvar">
            <xsl:choose>
                <xsl:when test="$mtfname = 'SetBaseType'">
                    <xsl:text>A data type for optional amplification and narrative information at teh beginning of each set.</xsl:text>
                </xsl:when>
                <xsl:when test="$change/@niemtypedoc!=''">
   ...
   accordance with provisions of DOD Directive 5230.25.</xsl:text>
        ...
        <Set niemelementname="{$niemelementnamevar}" niemtype="{$niemcomplextypevar}" niemelementdoc="{$niemelementdocvar}" mtfname="{@name}" mtfdoc="{$mtfdocvar}" niemtypedoc="{$niemtypedocvar}">
            ...
            <xsl:apply-templates select="*[not(contains(name(), ':annotation'))]">
                ...
                <xsl:with-param name="niemsettypevar" select="$niemcomplextypevar"/>
            </xsl:apply-templates>
        </Set>
    </xsl:template>
   ...
   accordance with provisions of DOD Directive 5230.25.</xsl:text>
    ...
    <xsl:template match="*:element">
        ...
        <xsl:variable name="setidvar">
            <xsl:value-of select="$baseline_sets_xsd/*:complexType[@name=$mtftypevar]/xs:annotation/xs:appinfo/SetFormatIdentifier"/>
        </xsl:variable>
        ...
        <xsl:variable name="annot">
            <xsl:choose>
                <xsl:when test="@name='Amplification'">
                    <xsl:apply-templates select="$baseline_sets_xsd/*[@name = 'AmplificationType']/*:annotation"/>
                </xsl:when>
                <xsl:when test="@name='NarrativeInformation'">
                    <xsl:apply-templates select="$baseline_sets_xsd/*[@name = 'NarrativeInformationType']/*:annotation"/>
                </xsl:when>
        ...
        <xsl:variable name="niemmatch">
            <xsl:choose>
                <!--<xsl:when test="$isSet">
                    <xsl:copy-of select="$niem_sets_map/Set[@mtfname = $mtftypevar][1]"/>
                </xsl:when>-->
        ...
        <xsl:variable name="fieldidvar">
            <xsl:value-of select="$niemmatch/*/info/inf:Field/@fieldid"/>
        </xsl:variable>
   ...
   accordance with provisions of DOD Directive 5230.25.</xsl:text>
        ...
        <Element niemelementname="{$niemelementnamevar}" niemtype="{$niemtypevar}" mtfname="{@name}" mtfdoc="{$mtfdocvar}" niemtypedoc="{$niemtypedocvar}" niemelementdoc="{$niemelementdocvar}">
            ...
            <xsl:if test="string-length($fieldidvar) &gt; 0">
                <xsl:attribute name="fieldid">
                    <xsl:value-of select="$fieldidvar"/>
                </xsl:attribute>
            </xsl:if>
                ...
                <xsl:for-each select="$appinfovar/*">
                    <xsl:copy>
                        ...
                        <xsl:if test="string-length($settypevar) &gt; 0">
                            <xsl:attribute name="mtfsetname">
                                <xsl:value-of select="$settypevar"/>
                            </xsl:attribute>
                        </xsl:if>
                        <xsl:if test="string-length($niemsettypevar) &gt; 0">
                            <xsl:attribute name="setname">
                                <xsl:value-of select="$niemsettypevar"/>
                            </xsl:attribute>
                        </xsl:if>
                        <xsl:if test="string-length($setseq) &gt; 0">
                            <xsl:attribute name="setseq">
            ...
            <xsl:apply-templates select="*[not(contains(name(), ':annotation'))]">
                <xsl:with-param name="settypevar" select="$settypevar"/>
                    <xsl:with-param name="niemsettypevar" select="$niemsettypevar"/>
    ...
    <xsl:template match="*:element[*:complexType/*:choice]">
        ...
        <xsl:variable name="segmenttype">
            <xsl:value-of select="concat($segmentnamevar,'Type')"/>
        </xsl:variable>
        <xsl:variable name="msgnamevar">
            <xsl:value-of select="ancestor::*:element[*:annotation/*:appinfo/MtfName]/@name"/>
        </xsl:variable>
    ...
    <xsl:template match="*:sequence/*:choice">
        ...
        <xsl:variable name="substmatch">
            <xsl:choose>
                <xsl:when test="$substGrp_Changes/Choice[@substgrpname = $subgrpname][@parentname = $settypevar]">
                    <xsl:copy-of select="$substGrp_Changes/Choice[@substgrpname = $subgrpname][@parentname = $settypevar][1]" copy-namespaces="no"/>
                </xsl:when>
        ...
        <Element substgrpname="{concat($choicenamevar,'Abstract')}" messagename="{$msgnamevar}" segmentname="{$segmenttype}" substgrpdoc="{$substgrpdocvar}">
            <xsl:for-each select="@*[not(name() = 'name')]">
                <xsl:copy-of select="."/>
            </xsl:for-each>
    ...
    <xsl:template name="GenTextName">
        ...
        <xsl:choose>
            ...
            <xsl:when test="$n = 'SecurityAndDefenseRemarks'">
                <xsl:text>SecurityAndDefensesRemarksGeneralTextSet</xsl:text>
            </xsl:when>
            <xsl:when test="$n = '48hourOutlookForecast'">
                <xsl:text>FortyEightHourOutlookForecast</xsl:text>
            </xsl:when>
            <xsl:otherwise>
                <xsl:value-of select="concat($n, 'GeneralTextSet')"/>
            </xsl:otherwise>
        </xsl:choose>
    </xsl:template>
