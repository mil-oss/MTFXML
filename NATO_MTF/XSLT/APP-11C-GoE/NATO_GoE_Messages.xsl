...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xsd="http://www.w3.org/2001/XMLSchema" exclude-result-prefixes="xsd" version="2.0">
    ...
    <xsl:variable name="rulesdoc" select="'../../XSD/APP-11C-GoE/natomtf_goe_message_rules.xsd'"/>
    <!--*****************************************************-->
    ...
    <xsl:template match="xsd:element/xsd:annotation" mode="el">
        <xsl:copy copy-namespaces="no">
            <xsl:if test="not(xsd:documentation)">
                <xsd:documentation>
                    <xsl:value-of select="xsd:appinfo/*:MtfName"/>
                </xsd:documentation>
            </xsl:if>
    ...
    <xsl:template match="xsd:element/xsd:annotation" mode="ctype">
        <xsl:copy copy-namespaces="no">
            <xsl:if test="not(xsd:documentation)">
                <xsd:documentation>
                    <xsl:value-of select="xsd:appinfo/*:SetFormatPositionName"/>
                    <xsl:value-of select="xsd:appinfo/*:SegmentStructureName"/>
                </xsd:documentation>
            </xsl:if>
            <xsl:apply-templates select="*" mode="ctype"/>
        </xsl:copy>
    ...
    <xsl:variable name="global_ctypes">
        <xsl:for-each select="$ctypes/*/*//xsd:element[@name][not(@type)]">
        ...
        <xsl:for-each select="$ctypes/*/*//xsd:element[@name][@type]">
            <xsl:variable name="n">
                <xsl:apply-templates select="@name" mode="txt"/>
            </xsl:variable>
    ...
    <xsl:template name="main">
        <xsl:result-document href="{$outputdoc}">
            <xsd:schema xml:lang="en-US" xmlns:xsd="http://www.w3.org/2001/XMLSchema" xmlns="urn:int:nato:mtf:app-11(c):goe:mtf" targetNamespace="urn:int:nato:mtf:app-11(c):goe:mtf"
                xmlns:field="urn:int:nato:mtf:app-11(c):goe:elementals" xmlns:set="urn:int:nato:mtf:app-11(c):goe:sets" xmlns:seg="urn:int:nato:mtf:app-11(c):goe:segments"
