...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xsd="http://www.w3.org/2001/XMLSchema" exclude-result-prefixes="xsd"
    ...
    <xsl:template match="xsd:appinfo[child::*[starts-with(name(), 'Segment')]]">
        <xsl:param name="doc"/>
        <xsl:copy copy-namespaces="no">
            <xsl:element name="Segment" xmlns="urn:int:nato:mtf:app-11(c):goe:segments">
                <xsl:apply-templates select="@*"/>
    ...
    <xsl:template match="xsd:element[not(starts-with(@name, 'GeneralText'))]" mode="global">
        ...
        <xsl:choose>
            ...
            <xsl:when
                test="xsd:complexType/xsd:complexContent/xsd:extension[@base] and count(xsd:complexType/xsd:complexContent/xsd:extension[@base]/*) &lt; 2">
    ...
    <xsl:template match="xsd:extension[@base]" mode="global">
        ...
        <xsl:copy copy-namespaces="no">
            ...
            <xsl:attribute name="base">
                <xsl:choose>
                    <xsl:when test="$goe_sets_xsd/xsd:schema/xsd:complexType[@name = $match_new_name]">
                        <xsl:value-of select="concat('set:', $match_new_name)"/>
                    </xsl:when>
                    <xsl:when test="$goe_sets_xsd/xsd:schema/xsd:simpleType[@name = $match_new_name]">
                        <xsl:value-of select="concat('set:', $match_new_name)"/>
                    </xsl:when>
                    <xsl:otherwise>
    ...
    <xsl:template match="xsd:appinfo" mode="ref">
        ...
        <xsl:copy copy-namespaces="no">
            <xsl:element name="Field" namespace="urn:int:nato:mtf:app-11(c):goe:segments">
                <xsl:apply-templates select="*" mode="attr"/>
    ...
    <xsl:template match="*:SetFormatPositionNumber" mode="attr">
        ...
        <xsl:variable name="initpos">
            <xsl:value-of select="ancestor::xsd:complexType[1]/ancestor::xsd:element[1]/xsd:annotation/xsd:appinfo/*:InitialSetFormatPosition[1]"/>
        </xsl:variable>
