...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xsd="http://www.w3.org/2001/XMLSchema" xmlns:term="http://release.niem.gov/niem/localTerminology/3.0/"
    ...
    <xsl:template match="xsd:element" mode="segmentglobal">
        ...
        <Segment mtfname="{@name}" mtfmsg="{$mtfmsg}" niemcomplextype="{$niemcomplextype}" niemelementname="{$niemelementname}" niemelementdoc="{$niemelementdoc}" mtfdoc="{$mtfdoc}"
    ...
    <xsl:template match="xsd:element">
        ...
        <xsl:variable name="mtfroot">
            <xsl:choose>
                <xsl:when test="$segment_changes/Element[@mtfname = $mtfname]">
        ...
        <xsl:variable name="docchange" select="$segment_changes/Element[@mtfname = $mtfroot][@niemname = $niemname]"/>
        ...
        <Element mtfname="{@name}" segmentname="{$segmentname}" niemelementname="{$niemelementname}" mtfdoc="{$mtfdoc}">
            ...
            <xsl:if test="string-length($niemmatch/*/@niemtypedoc) &gt; 0">
    ...
    <xsl:template match="xsd:choice">
                ...
                <xsl:when test="$segname='SpecialOptionDataAsrSegment'">
                    <xsl:choose>
                        <xsl:when test="xsd:element[@name='UnitDesignationData']">
                            <xsl:text>SurfaceGroundChoice</xsl:text>
                        </xsl:when>
                        <xsl:otherwise>
                            <xsl:text>SupplyRateChoice</xsl:text>
                        </xsl:otherwise>
                    </xsl:choose>
                </xsl:when>
        ...
        <xsl:variable name="segchoicename">
            <xsl:value-of select="concat($segmentname, 'Choice')"/>
        </xsl:variable>
        <xsl:variable name="substmatch">
            <xsl:copy-of select="$substGrp_Changes/Choice[@choicename = $segchoicename][@segmentname = $segmentname][1]"/>
        </xsl:variable>
        ...
        <Element choicename="{$choicename}" segmentname="{$segmentname}" substgrpname="{$choicename}" substgrpdoc="{$substgrpdoc}" seq="{$seq}">
            <xsl:choose>
                <!--THIS MITIGATES AN ACTUAL INCONSISTENCY IN THE MIL STD-->
                <xsl:when test="$choicename = 'Link16UnitSegmentChoice'">
