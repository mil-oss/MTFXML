...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xsd="http://www.w3.org/2001/XMLSchema" xmlns:mtfappinfo="urn:mtf:mil:6040b:appinfo" exclude-result-prefixes="xsd" version="2.0">
    ...
    <xsl:variable name="composites">
        <xsl:for-each select="$composites_xsd/xsd:schema/xsd:complexType">
                ...
                <xsl:for-each select="xsd:sequence/xsd:element">
                    ...
                    <Ref>
                        ...
                        <xsl:attribute name="niemelementname">
                            <xsl:choose>
                                <xsl:when test="$fieldmap/Fields/*[@mtfname = $t]">
                                    <xsl:value-of select="$fieldmap/Fields/*[@mtfname = $t]/@niemelementname"/>
                                </xsl:when>
                                <xsl:when test="$n = 'BlankSpace'">
                                    <xsl:text>f:BlankSpaceText</xsl:text>
                                </xsl:when>
                            </xsl:choose>
                        </xsl:attribute>
            ...
            <Composite mtfname="{@name}" niemcomplextype="{$niemcomplextype}" niemelementname="{$niemelementname}" niemelementdoc="{$niemelementdoc}" mtfdoc="{$mtfdoc}" niemtypedoc="{$niemtypedoc}">
                <appinfo>
    ...
    <xsl:variable name="compositexsd">
        <xsl:for-each select="$composites/Composite">
            ...
            <xsd:complexType name="{@niemcomplextype}">
                ...
                <xsd:complexContent>
                    <xsd:extension base="structures:ObjectType">
                        <xsd:sequence>
                            <xsl:for-each select="*:Sequence/Ref">
            ...
            <xsd:element name="{@niemelementname}" type="{@niemcomplextype}" nillable="true">
                <xsd:annotation>
                    ...
                    <xsl:for-each select="appinfo">
                        <xsd:appinfo>
                            <mtfappinfo:Field name="{*:Field/@name}" explanation="{*:Field/@explanation}" version="{*:Field/@version}"/>
                        </xsd:appinfo>
                    </xsl:for-each>
                </xsd:annotation>
            </xsd:element>
    ...
    <xsl:template name="main">
        <xsl:result-document href="{$compositeoutputdoc}">
            <xsd:schema xmlns="urn:mtf:mil:6040b:niem:composites" xmlns:ism="urn:us:gov:ic:ism:v2" xmlns:xsd="http://www.w3.org/2001/XMLSchema"
                ...
                xmlns:ddms="http://metadata.dod.mil/mdr/ns/DDMS/2.0/" xmlns:f="urn:mtf:mil:6040b:niem:fields" targetNamespace="urn:mtf:mil:6040b:niem:composites"
                ...
                <xsd:import namespace="urn:mtf:mil:6040b:niem:fields" schemaLocation="NIEM_MTF_Fields.xsd"/>
                <xsd:annotation>
                    <xsd:documentation>
        ...
        <xsl:result-document href="{$compositemapoutputdoc}">
            <Composites>
                <xsl:for-each select="$composites">
                    <xsl:sort select="@name"/>
                    <xsl:copy-of select="."/>
                </xsl:for-each>
            </Composites>
