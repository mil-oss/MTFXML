...
function getRecord(dbstorename, key, callback) {
    ...
    dbRequest.error = function (evt) {
        console.log("IndexedDB Error retrieving " + key);
    }
}
//getRecordURL
function getRecordURL(dbstorename, key, callback) {
    //console.log("getRecord: " + key);
    var tx = Ldb.transaction(dbstorename, 'readonly');
    var store = tx.objectStore(dbstorename);
    dbRequest = store.get(key);
    dbRequest.onsuccess = function (evt) {
        var dUrl = URL.createObjectURL(new Blob([evt.target.result.data], {
            type: 'text/xml'
        }))
        if (typeof callback === "function") {
            callback(dUrl);
