...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xsd="http://www.w3.org/2001/XMLSchema" xmlns:mtfappinfo="urn:int:nato:ncdf:mtf:appinfo" version="2.0">
    ...
    <xsl:template name="test">
        <Test>
            <xsl:apply-templates select="$baseline_msgs_xsd/*[@name = 'FrequencyManagement']/xsd:complexType/xsd:sequence/xsd:choice[2]"/>
        </Test>
    </xsl:template>
        ...
        <xsl:variable name="substgrpdocvar">
            <xsl:choose>
                <xsl:when test="$ncdfelementdocvar">
                    <xsl:apply-templates select="concat('A data concept for a choice of ', lower-case($ncdfelementdocvar))" mode="normlize"/>
        ...
        <xsl:variable name="substgrpncdfdoc">
            <xsl:choose>
                ...
                <xsl:when test="string-length($substgrpdocvar) &gt; 0">
                    <xsl:apply-templates select="concat('A data concept for a substitution group for ', lower-case($substgrpdocvar))" mode="normlize"/>
                </xsl:when>
            </xsl:choose>
        </xsl:variable>
    ...
    <xsl:template match="xsd:sequence/xsd:choice">
        ...
        <xsl:param name="segmentnamevar"/>
        <xsl:param name="messagenamevar"/>
        <messagenamevar><xsl:value-of select="$messagenamevar"/></messagenamevar>
        <xsl:variable name="msgnamevar">
            <xsl:value-of select="ancestor::xsd:element[parent::xsd:schema][1]/@name"/>
        </xsl:variable>
        <msgnamevar><xsl:value-of select="$msgnamevar"/></msgnamevar>
        <xsl:variable name="parentnamevar">
            <xsl:choose>
        ...
        <parentnamevar><xsl:value-of select="$parentnamevar"/></parentnamevar>
        ...
        <subname><xsl:value-of select="$subname"/></subname>
        ...
        <subgrpname><xsl:value-of select="$subgrpname"/></subgrpname>
        <xsl:variable name="substmatch">
            <xsl:choose>
                <xsl:when test="$substGrp_Changes/Choice[@mtfname = $subgrpname][@parentname = $parentnamevar]">
                    <xsl:copy-of select="$substGrp_Changes/Choice[@mtfname = $subgrpname][@parentname = $parentnamevar][1]"/>
                </xsl:when>
                <xsl:when test="$substGrp_Changes/Choice[@mtfname = $subgrpname][@segmentname = $segnamevar]">
                    <xsl:copy-of select="$substGrp_Changes/Choice[@mtfname = $subgrpname][@segmentname = $segnamevar][1]"/>
                </xsl:when>
                <xsl:when test="$substGrp_Changes/Choice[@mtfname = $subgrpname][@parentname = '']">
                    <xsl:copy-of select="$substGrp_Changes/Choice[@mtfname= $subgrpname][@parentname = ''][1]"/>
                </xsl:when>
        ...
        <substmatch><xsl:copy-of select="$substmatch"/></substmatch>
