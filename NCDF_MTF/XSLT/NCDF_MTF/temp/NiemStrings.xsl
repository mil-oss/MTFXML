...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" 
    ...
    <xsl:variable name="strings_xsd" select="document('../../XSD/APP-11C-ch1/Consolidated/fields.xsd')/xsd:schema/xsd:simpleType[xsd:restriction[@base = 'xsd:string']/xsd:pattern]"/>
    ...
    <xsl:template match="xsd:simpleType" mode="maptype">
        ...
        <xsl:variable name="ncdfsimpletype" select="concat($ncdfelementnamevar, 'SimpleType')"/>
        ...
        <String mtfname="{@name}" ncdfsimpletype="{$ncdfsimpletype}" ncdfcomplextype="{$ncdfcomplextype}" ncdfelementname="{$ncdfelementnamevar}" base="xsd:string" pattern="{$pattern}"
            ncdfpattern="{$ncdfpattern}" ncdfelementdoc="{$ncdfelementdoc}" mtfdoc="{$mtfdoc}" ncdftypedoc="{$ncdftypedocvar}">
