 ...
 */
-->
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:nsm="urn:int:nato:ncdf:nsm" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:exsl="http://exslt.org/common" exclude-result-prefixes="xs" version="1.0">
    ...
    <xsl:variable name="TestData" select="exsl:node-set(document('../../XSD/IEPD/xml/instance/common/mtf-testdata.xml')/TestData)"/>
    ...
    <xsl:template match="xs:schema/xs:element" mode="root">
        ...
        <xsl:element name="{@name}" xmlns="urn:int:nato:ncdf:mtf" xmlns:nsm="urn:int:nato:ncdf:nsm" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance">
            <xsl:attribute name="xsi:schemaLocation">
                <xsl:value-of select="concat('urn:int:nato:ncdf:mtf ',$xsdv)"/>
            </xsl:attribute>
            <xsl:apply-templates select="//xs:schema/xs:complexType[@name=$t]">
            ...
            <xsl:variable name="testvalue">
                <xsl:choose>
                    <xsl:when test="$TestData//*[name() = $elnode]">
                        <xsl:value-of select="$TestData/*[name() = $elnode]/Test[@valid = 'true']/@value"/>
                    </xsl:when>
                    <xsl:when test="$TestData//*[name() = $typbase]">
                        <xsl:value-of select="$TestData/*[name() = $typbase]/Test[@valid = 'true']/@value"/>
                    </xsl:when>
                    <xsl:when test="$TestData//*[name() = $simplebase]">
                        <xsl:value-of select="$TestData/*[name() = $simplebase]/Test[@valid = 'true']/@value"/>
                    </xsl:when>
                </xsl:choose>
            </xsl:variable>
                       ...
                       <xsl:attribute name="typbase">
                        ...
                        <xsl:choose>
                            <xsl:when test="$basenode/@name = 'ResponseWithExplanationType'">
                                <xsl:attribute name="nsm:classification">
                                    <xsl:text>U</xsl:text>
                                </xsl:attribute>
                                <xsl:attribute name="nsm:ownerProducer">
